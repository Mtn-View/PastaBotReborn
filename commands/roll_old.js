	...
	catchCommand(bot, message, args) {
		...
		if (args[0] === (cmd.match(re) || {}).input) {
			...
			if (args[0] === 'drop' && typeof (args[1]) === Number) {
				...
				ch.send({ content: `${cmd} = ${customRoll.sum}` })
			}
			...
			if (args[1] === 'v') {
				...
				ch.send({ content: `${message.member} here are your *verbose* stats: \n ${verboseOut}` })
			} else if (args[1] === 'legit' && isOwner) {
				ch.send({ content: `${message.member} here are your *legit* stats: \n **18 18 18 18 18 18**` })
			} else if (args[1] === 'super' && args[2] === 'legit' && isOwner) {
				ch.send({ content: `${message.member} here are your *super legit* stats: \n **20 20 20 20 20 20**` })
			} else {
				let out = ''
				for (let i = 0; i < 6; i++) {
					out += `${statArray[i].sum } `
				...
				if (args[1]) {
					...
					ch.send({ content: `${message.member} here are your *${argsString}* stats: \n **${out}**` })
				} else {
					ch.send({ content: `${message.member} here are your stats: \n **${out}**` })
				}
			...
			ch.send({ content: `Unknown command \`roll ${args}\`` })
