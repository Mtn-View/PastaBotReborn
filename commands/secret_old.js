...
var methods = {
	catchCommand(discord, message, args) {
		...
		if (args[0] === 'reset' && isOwner) {
			ch.send({ content: `Resetting all secrets... :arrows_counterclockwise:` })
			this.resetAllSecrets()
		// enable
		} else if (args[0] === 'enable' && isOwner) {
			if (this.allowSecretDraws(true)) {
				ch.send({ content: `Secret draws enabled. :shushing_face:` })
			} else {
				ch.send({ content: `Failed to enable secret draws...?` })
			}
		// disable
		} else if (args[0] === 'disable' && isOwner) {
			if (!this.allowSecretDraws(false)) {
				ch.send({ content: `Secret draws disabled. :no_entry_sign:` })
			} else {
				ch.send({ content: `Failed to disable secret draws...?` })
			}
		// remaining
		} else if (args[0] === 'remaining' && isOwner) {
			ch.send({ content: `There are ${this.getSecretsRemaining()} unclaimed secrets remaining.` })
		} else if (args[0] === 'restore' && isOwner) {
			let restorePath = this.restoreBackupSecretJSON(args[1])
			if (restorePath) {
				ch.send({ content: `Restored database from \`${restorePath}\`.` })
			} else {
				ch.send({ content: `:thinking: Error restoring database from file...` })
			}
		// backup
		} else if (args[0] === 'backup' && isOwner) {
			let writePath = this.backupSecretJSON(args[1])
			if (writePath) {
				ch.send({ content: `Successfully written to \`${writePath}\`` })
			} else {
				ch.send({ content: `Backup successfully failed. No really. It didn't work. IDK why. I'm too lazy to write code to check how you fucked this up.` })
			}
		// all
		} else if (args[0] === 'all' && isOwner) {
			ch.send({ content: "Revealing everyone's secrets... Privately. :eyes:" })
			this.getAllClaimedSecrets(message)
				.then(result => {
					if (result) {
						message.author.send({ content: result })
						console.log(`s${result}s`)
					} else {
						message.author.send({ content: "FUCK" })
						console.log(`s${result}s`)
					}
				})
		// redraw
			...
			if (numSecretsRemoved > 0) {
				ch.send({ content: `Redrawing your secret... One moment, please... :timer:` })
			} else {
				ch.send({ content: `You don't have any secrets to redraw! Redrawing anyways... One moment, please... :timer:` })
			}
			setTimeout(() => {
		...
		// unclaim
			...
			ch.send({ content: `Unclaimed all your secrets.` })
		// count
		} else if (args[0] === 'count') {
			ch.send({ content: `You have ${this.getNumberSecretsClaimedByAuthor(message)} secrets.` })
		// draw
		} else if (args[0] === 'draw') {
			this.sendSecret(message, ch, discord)
		// "default"
		} else if (!args[0]) { // Default secret command with no args
			ch.send({ content: `Use \`${prefix}secret draw\` to draw a secret, or \`${prefix}help\` for help.` })
		} else { // just in case they fuck it up or try and reset without permissions
			ch.send({ content: `Unknown command \`${args}\` or insufficient permissions.` })
		}
	},
	loadSecretsJSON() {
	...
	async sendSecret(message, channel, discord) {
		...
		if (secretObject) {
			...
			channel.send({ content: `Secret drawing not enabled.` })
		}
	},
